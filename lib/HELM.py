...
def HELM(para, train_x, train_y=1):
    ...
    for helm in range(para['nhelm']):  # Number of trained HELM
        ...
        for i in range(number_of_AE_layers):  # Number of AE-Layers
            ...
            w = 2 * np.random.rand(x.shape[1], para['neuron_number'][
                i]) - 1  # Generate random weights, uniform distribution between -1 and 1
            h, _ = mmm.mapminmax(np.tanh(x.dot(w)),
                                 axis=1)  # Activation function and normalization, H hidden layer matrix
            model[helm]['beta'][i] = fista.FISTA_l2_l1(h, x, para['fista_weight'],
                                                       para['fista_cv'])  # argmin beta with FISTA
            x, model[helm]['ps'][i] = mmm.mapminmax(x.dot(model[helm]['beta'][i].T),
                                                    axis=1)  # T^Train; save xmin and xmax
        ...
        h = np.tanh(x.dot(w))
        model[helm]['beta'][number_of_AE_layers] = np.linalg.solve(
            (h.T.dot(h) + np.eye(h.shape[1]) * para['ridge_weight']), h.T.dot(train_y))
